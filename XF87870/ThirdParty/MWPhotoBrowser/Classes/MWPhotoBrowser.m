...
@property (nonatomic, strong) UILabel *picsNumLab;
...
- (UILabel *)picsNumLab {
    if (_picsNumLab == nil) {
        _picsNumLab = [[UILabel alloc] initWithFrame:CGRectMake((SCREEN_WIDTH - 100) / 2, 60, 100, 15)];
        _picsNumLab.font = [UIFont systemFontOfSize:15.0f];
        _picsNumLab.textColor = [UIColor whiteColor];
        _picsNumLab.textAlignment = NSTextAlignmentCenter;
    }
    return _picsNumLab;
}
...
- (void)viewDidLoad {
    ...
    // add pic lab
    [self.view addSubview:self.picsNumLab];
...
- (void)updateNavigation {
    ...
    if (_gridController) {
        ...
        if ([_delegate respondsToSelector:@selector(photoBrowser:titleForPhotoAtIndex:)]) {
            ...
            NSString *showStr = [NSString stringWithFormat:@"%lu / %lu", (unsigned long)(_currentPageIndex+1), (unsigned long)numberOfPhotos];
            if (self.isHideNav) {
                self.picsNumLab.text = showStr;
                [self.view bringSubviewToFront:self.picsNumLab];
            }else {
                self.title = showStr;
            }
        }
